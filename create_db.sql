...
CREATE TABLE chat ( id_chat SERIAL PRIMARY KEY, name VARCHAR(30) NOT NULL, description VARCHAR(30), room_id INTEGER, FOREIGN KEY (room_id) REFERENCES room(id_room) ON DELETE CASCADE
);

CREATE TABLE message ( id_message SERIAL PRIMARY KEY, content VARCHAR(1000), date_send TIMESTAMP, chat_id INTEGER, user_id INTEGER, FOREIGN KEY (chat_id) REFERENCES chat(id_chat) ON DELETE CASCADE, FOREIGN KEY (user_id) REFERENCES account(id_user) ON DELETE CASCADE
);
